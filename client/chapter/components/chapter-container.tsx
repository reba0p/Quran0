...
export default function ChapterContainer() {
  ...
  const searchFilterChapters = CHAPTERS_LIST.filter((chapter) => {
    const nameAr = chapter.name?.["ar"] || "";
    const nameLocale = chapter.name?.[locale] || "";
    const transliterationName = chapter.transliterationName || "";

    return (
      nameAr.includes(search) ||
      nameLocale.toLocaleLowerCase().includes(search) ||
      transliterationName.toLocaleLowerCase().includes(search)
    );
  });
    ...
    <div className="grid grid-cols-1 2xl:grid-cols-5 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 gap-5 justify-items-start md:my-3 mb-20 mt-1">
      ...
      {searchFilterChapters.map((chapter) => (
        <ChapterCard
          key={chapter.id}
          id={chapter.id}
          name={chapter.name?.[locale] || "Unknown"}
          transliterationName={chapter.transliterationName || "Unknown"}
